# Cloud Run: Telegram MTProto bridge

Эта папка уже готова как маленький Docker-сервис для Cloud Run. Он нужен сайту Gala View Report Collector, чтобы получать точные просмотры и реакции Telegram через MTProto.
...
```json
{"ok":true,"telegram_api_id":true,"telegram_api_hash":true,"telegram_session":true}
```
...
После этого кнопка обновления статистики будет использовать мост и возвращать точные целые числа для Telegram.
