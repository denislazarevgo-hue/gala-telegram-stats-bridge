...
С такими значениями мост проверяет до 2000 последних сообщений одного MAX-канала за одно обновление. Если публичный код поста из ссылки не совпал с данными API, мост дополнительно берёт текст публичной карточки MAX и сопоставляет его с текстами сообщений из API. `MAX_CHANNEL_CHAT_IDS` нужен, если MAX больше не отдаёт `chat_id` по публичной ссылке канала: укажите пары `channel:chat_id` через запятую. Если пост старше, увеличьте `MAX_CHAT_SCAN_PAGES`, но не ставьте слишком большое значение без необходимости.
